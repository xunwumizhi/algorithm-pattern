...
value 每个节点不一样，作为去重key
```go
func cloneGraph(node *Node) *Node {
    if node == nil {
        return nil
    }
    history := map[int]*Node{} // 存放clone结果
    queue := []*Node{node}
    for len(queue) > 0 {
        head := queue[0]
        queue = queue[1:]
        v, ok := history[head.Val]
        if !ok {
            v = &Node{Val: head.Val}
            history[head.Val] = v
        }
        ...
        for _, n := range head.Neighbors {
            tmp, ok := history[n.Val]
            if !ok {
                tmp = &Node{Val: n.Val}
                history[n.Val] = tmp
                queue = append(queue, n)
            }
            v.Neighbors = append(v.Neighbors, tmp)
        }
...
func updateMatrix(matrix [][]int) [][]int {
	// 层序遍历，先把0放前面，一层层计算距离，确定了的节点放入队列
    // 0,0,0|1,1|2,2，一层层缩小圈，直至遍历所有节点
	queue := make([][]int, 0)
